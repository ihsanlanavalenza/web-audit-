            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $inv->role === 'auditor' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700' }}">
                                {{ ucfirst($inv->role) }}
                            </span>
                        </td>
                        <td class="text-slate-600">{{ $inv->client?->nama_client ?? '-' }}</td>
                        <td>
                            <code class="text-xs bg-slate-100 px-2 py-1 rounded font-mono text-slate-600">{{ substr($inv->token, 0, 16) }}...</code>
                        </td>
                        <td>
                            @if($inv->accepted_at)
                        ...
                        <td>
                            <div class="flex gap-2">
                                <button onclick="navigator.clipboard.writeText('{{ url('/register?token=' . $inv->token) }}')" class="text-blue-600 hover:text-blue-700 text-xs font-medium">
                                    Salin Link
                                </button>
                                <button wire:click="deleteInvitation({{ $inv->id }})" wire:confirm="Yakin hapus undangan ini?" class="text-red-600 hover:text-red-700 text-xs font-medium">
                                    Hapus
                                </button>
                            </div>
                        </td>
