<div>
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex gap-2">
            @if($clients->count() > 0)
            <button wire:click="$set('showDeleteConfirm', true)" class="btn-ghost text-sm text-red-600 border-red-200 hover:bg-red-50" id="btn-delete-all">
                Hapus Semua
            </button>
            @endif
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex gap-2">
                                <button wire:click="editClient({{ $client->id }})" class="text-blue-600 hover:text-blue-700 text-xs font-medium">Edit</button>
                                <a href="{{ route('schedule.show', $client->id) }}" class="text-emerald-600 hover:text-emerald-700 text-xs font-medium">Schedule</a>
                                <button wire:click="deleteClient({{ $client->id }})" wire:confirm="Yakin hapus klien ini?" class="text-red-600 hover:text-red-700 text-xs font-medium">Hapus</button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
    ...
    <div class="modal-overlay" wire:click.self="$set('showModal', false)">
        <div class="modal-content p-8">
            <h3 class="text-lg font-bold mb-6 text-slate-900">{{ $editId ? 'Edit Klien' : 'Tambah Klien Baru' }}</h3>
            <form wire:submit="save" class="space-y-4">
                <div>
                    <label class="form-label">Nama Client</label>
                    <input wire:model="nama_client" type="text" class="form-input" placeholder="PT Contoh Indonesia">
                    @error('nama_client') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div>
                    ...
                    @error('no_contact') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div>
                    ...
                    @error('tahun_audit') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
    ...
    <div class="modal-overlay" wire:click.self="$set('showDeleteConfirm', false)">
        <div class="modal-content p-8 max-w-sm text-center">
            ...
            <p class="text-sm text-slate-500 mb-6">Tindakan ini tidak dapat dibatalkan. Semua data klien dan request terkait akan dihapus.</p>
